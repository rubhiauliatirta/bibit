  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="flex">
      <div className={`bg-gray-200 rounded-l-lg w-full flex items-center border-2 
        ${isFocus ? 'border-blue-500' : 'border-gray-200'} `}>
          <input 
          ...
          />
        <svg className={`animate-spin -ml-1 mr-3 h-5 w-5 text-blue-500 ${!isLoading ? 'hidden' : ''}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
      </div>
      <input type="submit" value="Search" className="rounded-r-lg bg-yellow-500 hover:bg-yellow-600 px-4 cursor-pointer"/>
    </form>
    {
      (suggestions !== null) && (
        <ul class="bg-white border border-gray-100 w-full md:w-3/4 mt-2 absolute z-20">
        {
          suggestions.length === 0 ? (
            ...
            suggestions.map(suggestion => (
              <li 
                ...
                class="pl-6 pr-2 py-3 border-b-2 border-gray-100 relative cursor-pointer hover:bg-blue-100">
                <BoldQuery
